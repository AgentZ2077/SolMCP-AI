...
mcp = FastMCP("SolMCP-AI")


# Define input model
class SwapInput(BaseModel):
    token_in: str = Field(
        description="Input token symbol (e.g., SOL)", examples=["SOL", "USDC"]
    )
    token_out: str = Field(
        description="Output token symbol (e.g., DARK)", examples=["DARK", "SOL"]
    )
    amount: float = Field(
...
def jupiter_swap(
    token_in: str, token_out: str, amount: float, tx_sender_pubkey: str
) -> SwapResponse:
    """
    ...
    try:
        ...
        return SwapResponse(success=True, result=response.json())
        ...
        return SwapResponse(success=False, error=error_message)
    except Exception as e:
        return SwapResponse(success=False, error=f"Unexpected error: {str(e)}")


# Set up description for the server
mcp.description = """
SolMCP-AI enables token swaps on Jupiter DEX via Dialect Blink API.
